...
package object impl {
  ...
  trait AkkaHttpRequestSigner[F[_]]
      extends HttpSigner[HttpRequest, F]
  ...
  trait AkkaHttpRequestVerifier[F[_]]
      extends HttpVerifier[HttpRequest, F]
      with RequestHelpers[F] {
    implicit val mat: Materializer
    implicit val ctx: ContextShift[F]
    implicit val F: Async[F]
  ...
  trait AkkaHttpResponseSigner[F[_]]
      extends HttpSigner[HttpResponse, F]
  ...
  trait AkkaHttpResponseVerifier[F[_]]
      extends HttpVerifier[HttpResponse, F]
      with ResponseHelpers[F] {
    implicit val mat: Materializer
    implicit val ctx: ContextShift[F]
    implicit val F: Async[F]
